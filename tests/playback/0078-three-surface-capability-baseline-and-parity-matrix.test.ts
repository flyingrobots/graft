...
describe("0078 three-surface capability baseline and parity matrix", () => {
  ...
  it("Are the current CLI-only, MCP-only, and API-only exceptions obvious rather than surprising?", () => {
    ...
    expect(CAPABILITY_REGISTRY.filter((capability) => capability.surfaces.join("+") === "api+mcp")).toHaveLength(25);
    ...
    expect(content).toContain("- `25` API + MCP capabilities");
    ...
    expect(content).toContain("- `25` intentionally API + MCP-only agent/control-plane capabilities");
  });
  ...
  it("Does the matrix name composed CLI operator surfaces instead of hiding them as MCP-only?", () => {
    ...
    expect(CAPABILITY_REGISTRY.find((capability) => capability.id === "daemon_status")).toMatchObject({
