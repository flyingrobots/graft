...
import type { PostHookInput } from "../../../src/hooks/posttooluse-read.js";
...
function makeInput(filePath: string, cwd?: string): PostHookInput {
  return {
...
describe("hooks: posttooluse-read", () => {
  ...
  it("no feedback for small files — Read was the right call", async () => {
    const output = await handlePostReadHook(makeInput(path.join(FIXTURES, "small.ts")));
  ...
  it("educates on large file reads with context cost", async () => {
    const output = await handlePostReadHook(makeInput(path.join(FIXTURES, "large.ts")));
    expect(output.exitCode).toBe(0);
  ...
  describe("non-parseable files", () => {
    ...
    it("no feedback for large non-JS/TS files", async () => {
      ...
      const output = await handlePostReadHook(makeInput(path.join(tmpDir, "big.md"), tmpDir));
      expect(output.exitCode).toBe(0);
      expect(output.stderr).toBe("");
    });
  });
  ...
  // -----------------------------------------------------------------------
  // No feedback for nonexistent files
  // -----------------------------------------------------------------------
  it("no feedback for nonexistent files", async () => {
    const output = await handlePostReadHook(makeInput("/nonexistent/file.ts"));
    expect(output.exitCode).toBe(0);
    expect(output.stderr).toBe("");
  });
  ...
  it("always exits 0", async () => {
    const small = await handlePostReadHook(makeInput(path.join(FIXTURES, "small.ts")));
    const large = await handlePostReadHook(makeInput(path.join(FIXTURES, "large.ts")));
    const missing = await handlePostReadHook(makeInput("/nope"));
