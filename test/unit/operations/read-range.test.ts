...
describe("operations: read_range", () => {
  it("returns requested line range with line numbers", async () => {
    const result = await readRange(fixturePath("medium.ts"), 1, 10, { fs: nodeFs });
  ...
  it("refuses ranges exceeding 250 lines", async () => {
    const result = await readRange(fixturePath("large.ts"), 1, 300, { fs: nodeFs });
  ...
  it("clips to file end if range extends past EOF", async () => {
    const result = await readRange(fixturePath("small.ts"), 1, 1000, { fs: nodeFs });
    expect(result.content).toBeDefined();
    expect(result.clipped).toBe(true);
  });

  it("returns error for nonexistent file", async () => {
    const result = await readRange(fixturePath("nope.ts"), 1, 10, { fs: nodeFs });
    expect(result.reason).toBe("NOT_FOUND");
  });

  it("returns error for invalid range (start > end)", async () => {
    const result = await readRange(fixturePath("small.ts"), 10, 5, { fs: nodeFs });
    expect(result.reason).toBeDefined();
  });

  it("includes path in result", async () => {
    const filePath = fixturePath("medium.ts");
