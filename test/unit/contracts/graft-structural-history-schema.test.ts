...
function writeSchemaArtifactFixture(options: {
  ...
  readonly generatedCodecText?: string;
} = {}): string {
  ...
  fs.writeFileSync(path.join(workspaceRoot, generatedCodecPath), generatedCodecText);
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      generatedCodecSha256: sha256(generatedCodecText),
...
function writeFakeWesley(
  ...
  const script = [
    ...
    `const generatedCodecText = ${JSON.stringify(options.generatedCodecText ?? "/* @generated by wesley-emit-typescript le-binary. Do not edit. */\n")};`,
    ...
    "  process.exit(0);",
    "}",
    'if (args[0] === "emit" && args[1] === "le-binary-typescript") {',
    '  if (args.length !== 8) { console.error(`emit le-binary-typescript expected 8 args, got ${args.length}`); process.exit(64); }',
    '  requireSchemaArgs("emit le-binary-typescript");',
    '  const outIndex = args.indexOf("--out");',
    '  if (outIndex === -1 || args[outIndex + 1] === undefined) {',
    '    console.error("missing --out");',
    "    process.exit(64);",
    "  }",
    '  if (args[outIndex + 2] !== "--codec-import" || args[outIndex + 3] !== "../echo/codec-runtime.js") {',
    '    console.error(`unexpected codec import args: ${args.join(" ")}`);',
    "    process.exit(64);",
    "  }",
    "  const outPath = args[outIndex + 1];",
    "  fs.mkdirSync(path.dirname(outPath), { recursive: true });",
    "  fs.writeFileSync(outPath, generatedCodecText);",
    "  process.exit(0);",
    "}",
...
describe("Graft structural history schema authority", () => {
  ...
  it("regenerates and byte-compares generated TypeScript with configured Wesley", () => {
    ...
    const generatedCodecText = "/* @generated by wesley-emit-typescript le-binary. Do not edit. */\n";
    const workspaceRoot = writeSchemaArtifactFixture({ generatedText, generatedCodecText });
    const wesleyBin = writeFakeWesley(workspaceRoot, { generatedText, generatedCodecText });
    ...
    try {
      ...
      expect(result.hermeticWesley).toEqual({
        ...
        regeneratedCodecSha256: sha256(generatedCodecText),
      });
      expect(readFakeWesleyInvocations(workspaceRoot)).toEqual([
        ...
        [
          ...
          path.join(workspaceRoot, "schemas/schema.graphql"),
          "--out",
          expect.stringContaining("graft-structural-history.codec.generated.ts"),
          "--codec-import",
          "../echo/codec-runtime.js",
        ],
      ]);
    } finally {
      fs.rmSync(workspaceRoot, { recursive: true, force: true });
  ...
  it("fails when Wesley emits a different generated codec", () => {
    const committedGeneratedCodecText =
      "/* @generated by wesley-emit-typescript le-binary. Do not edit. */\nexport const codec = 1;\n";
    const regeneratedCodecText =
      "/* @generated by wesley-emit-typescript le-binary. Do not edit. */\nexport const codec = 2;\n";
    const workspaceRoot = writeSchemaArtifactFixture({ generatedCodecText: committedGeneratedCodecText });
    const wesleyBin = writeFakeWesley(workspaceRoot, { generatedCodecText: regeneratedCodecText });
    ...
    try {
      expect(
        ...
        expect.stringContaining(
          "src/generated.codec.ts does not match Wesley regenerated codec output for schemas/schema.graphql",
        ),
      ]);
    } finally {
      fs.rmSync(workspaceRoot, { recursive: true, force: true });
