...
import { observe, commitsLens } from "./observers.js";
...
async function getCommitTick(
  ...
  const obs = await observe(warp, commitsLens());
...
export async function checkStaleDocs(
  ...
  const docTick = await getCommitTick(warp, docCommitSha);

  const staleSymbols: StaleSymbolEntry[] = [];
  const unknownSymbols: string[] = [];

  for (const symbolName of symbolNames) {
    const history = await commitsForSymbol(warp, symbolName);
    ...
    for (const commit of history.commits) {
      const commitTick = await getCommitTick(warp, commit.sha);
