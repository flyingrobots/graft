...
| Generated codecs | `src/generated/graft-structural-history.codec.generated.ts` | Wesley 0.1.0 `le-binary` TypeScript emitter output for Graft's schema: per-operation var encoders/decoders, wire-compatible with Rust `echo_wasm_abi::codec` value layout. Runtime `op_id` authority is not owned by the value codec emitter. Graft's current fake Echo witness keeps an internal EINT op-id constant only to exercise the byte seam; the real operation identity authority belongs in Echo's released contract-host generator/artifact set. |
